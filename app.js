(() => {
  ...
  function drawRoadmapPath() {
    ...
    stop1.setAttribute("offset", "0%"); stop1.setAttribute("stop-color", "var(--flow)");
    const stop2 = document.createElementNS(NS, "stop");
    stop2.setAttribute("offset", "100%"); stop2.setAttribute("stop-color", "var(--amber)");
    ...
    if (aheadPts.length >= 2) svg.appendChild(makePath(segmentPath(aheadPts), "var(--dim)", "7"));
    ...
    if (testSoundBtn) {
      testSoundBtn.addEventListener("click", () => {
        ...
        setTimeout(() => {
          ...
          if (!Ctx) { diagEl.textContent = "Web Audio API не поддерживается этим браузером."; return; }
          const ctx = beep._ctx;
          diagEl.textContent = ctx
            ? `Audio: state=${ctx.state}, sampleRate=${ctx.sampleRate}, muted=${soundMuted}, error=${_lastBeepError || "нет"}`
            : "AudioContext ещё не создан.";
        }, 250);
      });
    }
